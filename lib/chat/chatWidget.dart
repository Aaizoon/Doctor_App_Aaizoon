...
//  }
  ...
  static Widget userListbuildItem(BuildContext context, String currentUserId, DocumentSnapshot document) {
            ...
            children: <Widget>[
              ...
              ConstrainedBox(
                ...
                child: new DecoratedBox(
                  decoration: new BoxDecoration(color: document.get('online') == 'online' ? Colors.greenAccent : Colors.transparent),
                ),
              ),
            ],
          ),
          onPressed: () {
            print("ci " + currentUserId);
            print("di " + document.id);
            print("name " + document.get('nickname'));
            print("name " + document.get('photoUrl'));
            // Navigator.push(
            //     context,
            //     MaterialPageRoute(
  ...
  static Widget widgetChatBuildListMessage(groupChatId, listMessage, currentUserId, peerAvatar, listScrollController) {
    return Flexible(
      child: groupChatId == ''
          ...
          : StreamBuilder(
              stream: FirebaseFirestore.instance.collection('messages').doc(groupChatId).collection(groupChatId).orderBy('timestamp', descending: true).limit(20).snapshots(),
  ...
  static Widget chatText(String chatContent, String id, var listMessage, int index, bool logUserMsg) {
    return Container(
      child: chatContent != "You requested a Video Call"
          ? Text(
              chatContent,
              style: TextStyle(color: logUserMsg ? primaryColor : Colors.white),
            )
          : Text(
              "Your Patient requested a Video Chat",
              style: TextStyle(color: logUserMsg ? primaryColor : Colors.white),
            ),
